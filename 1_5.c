...
char s3[MAX][50];
int path[MAX];
void  change(adjmatrix *g,adjlist *g1);

void window_choose();
void choose_(adjmatrix *g,adjlist *g1);		//选择线路
void print_map();		
...
void short_address_router(adjmatrix *g,adjlist *g1);	//输出中转点最少路径
void dijkstra(adjmatrix *g);		//输出最短距离路径
void print_prim(adjmatrix *g);		//
void print_quanzhi(adjmatrix *g,adjlist *g1);	//打印临接矩阵
...
int FirstAdjVex(adjlist *g1,int v0);
int adjvex_(adjlist *g1,int v0,int w);
int main(void)
{
	adjmatrix g;
	adjlist g1;
...
void  change(adjmatrix *g,adjlist *g1)
{
		int i,j,k,vex1,vex2,x;
		arcnode *p,*q;
		g1->vexnum=g->vexnum;
		g1->arcnum=g->vexnum;
		for(i=0;i< g1->vexnum;i++)
		{
				strcpy(g1->vertex[i].vexdata,g->vex[i].name);
				g1->vertex[i].first=(struct arc*)malloc(sizeof(arcnode));
				p=g1->vertex[i].first;
				for(j=0;j<g1->vexnum;j++)
				{
					if(g->arcs[i][j]!=INFINITY)
					{
						p->adjvex=j;
						p->wei=g->arcs[i][j];
						p->next=(struct arc*)malloc(sizeof(arcnode));
						q=p;
						p=p->next;
					}
					//q=(struct arc*)malloc(sizeof(arcnode));
...
int FirstAdjVex(adjlist *g1,int v0)
{
	arcnode *p=g1->vertex[v0].first;
	p=p->next;
	while(p)
	{
		if(visited[p->adjvex]==0&&p->wei!=INFINITY)
			return p->adjvex;
		p=p->next;
	}
	return -1;
...
void short_address_router(adjmatrix *g,adjlist *g1)		//输出中转点最少路径
{
	...
	for(i=0;i<g1->vexnum;i++)
		if(strcmp(s1,g1->vertex[i].vexdata)==0)
		{
			v0=i;
			break;
		}
	...
	for(i=0;i<g1->vexnum;i++)
		if(strcmp(s2,g1->vertex[i].vexdata)==0)
		{
			v=i;
			break;
		}
        ...
        while(w != -1)
        {
            if(!visited[w])
            {
                ...
                if(w == v)
                {
                    ...
                    printf("-->%s",g->vertex[p->adjvex].vexdata);
                    if(p->adjvex == t1)
                    {
                        break;
                    }
                    p = g->vertex[p->adjvex].first;
                    q = p;
                    }
                }
		in_queue(q1,w);
...
void print_quanzhi(adjmatrix *g,adjlist *g1)	//打印临接矩阵
{
	...
	for(i=0;i<g1->vexnum;i++)
	{
		...
		//	for(j=0;j<g1->vexnum;j++)
			//	if(p->adjvex==j)
			//	{
				if(p->adjvex==INFINITY)  break;
					printf("-->%s",g1->vertex[p->adjvex].vexdata);
					//break;
				//}
			p=p->next;
		}
...
{
	FILE *fp,*fp1;
	fp=fopen("xiaoyuantu1","rw");
	fp1=fopen("xiaoyuantu1","rw");
	char s1[300];
/*printf("\t╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬┣  \t╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬╬┣┫\n");
printf("\t     ┏━━━━ ━━━┓      ┏━━━━ ━━━┓ ┣  \t┣                                               ┫\n");
printf("\t     ┃        ┃      ┃        ┃ ┣  \t┣            ┏━━━━━━┓                           ┫\n");
...
fprintf(fp,"\t   ┣      ┣              ║  ║   ┣大\t┣┏⑧  ━┫                        ┏ ━━━┻━┓         ┫\n");
fprintf(fp,"\t   ┣      ┣    ┏ ━━━┻━┓  ║  ║   ┣  \t┣┃大学┃                        ┃   实 ┃         ┫\n");
fprintf(fp,"\t   ┣      ┣    ┃14 饭 ┃  ║  ║   ┣  \t┣┃生活┃         ┏━ ━━━━━━━┓    ┃ ⑩ 验 ┃         ┫\n");
fprintf(fp,"\t   ┣      ┣    ┃   堂 ┃  ║  ║   ┣  \t┣┃动中┣════      ⑨喷泉 ┣═ ┫        楼 ┃         ┫\n");
fprintf(fp,"\t15 ┣      ┣      超市 ┃  ║  ║   ┣  \t┣┃ 心 ┃         ┗ ━━━  ┳  ┛    ┗━━┳━ ━┛         ┫\n");
fprintf(fp,"\t┏━  ━━┓   ┣    ┗━━┳━ ━┛  ║  ║   ┣道\t┣┗  ━┳┛                ║          ║             ┫\n");
fprintf(fp,"\t┃ 宿舍┃   ┣              ║  ║   ┣  \t┣    ║                 ║      ┏━ ━┻ ━━┓         ┫\n");
fprintf(fp,"\t┗━  ━━┛    ══════════════       ┣  \t┣    ║                 ║      ┃⑾教学楼┃         ┫\n");
...
void choose_(adjmatrix *g,adjlist *g1)
{
	int x;
	char ch;
	printf("请输入您的选择:");
	...
	while(scanf("%d",&x))
	{
		switch(x)
		{
			...
			case 6:
				...
				break;
			case 7:
				short_address_router(&*g,g1);	//打印最少节点路线
				printf("\n输入任意键返回:\n");
				while(getchar()!='\n')
					continue;
				getchar();
